	...
	<section>
		...
		<article>
			<?php
				...
				$query = "select year from facelog where userid='$slamuserid' order by year";
				$run = mysqli_query($con,$query);
				if(mysqli_num_rows($run) > 0){
					while($row = mysqli_fetch_array($run)){
						$totalAvailableYear = $row['year'];
						unset($totalYear[$totalAvailableYear]);
					}
				}
			?>
			<div class="addp">
				...
				<div class="flashimg" style="display: block;">
					...
					<select name="year" id="year">
						...
						<?php foreach($totalYear as $k => $v){ ?>
							<option value="<?= $k; ?>"><?= $k; ?> Year, <?= $v; ?></option>
						<?php } ?>
					</select>
	...
	<script>
		...
		$('#facelogBtn').on('click',function(e){
			...
			$.ajax({
				url : "ajax/addlog",
				type : "POST",
				data : {year: year, image: image, imgtext: imgtext},
				dataType: 'JSON',
				cache: false,
				beforeSend: function(){
					$("#cover").show();
